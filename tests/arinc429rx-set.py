...
def set_arinc429rx(flags, priority_labels=bytes(3), label_filters=bytes(32)):
    with socket.socket(socket.AF_NETLINK, socket.SOCK_RAW, socket.NETLINK_ROUTE) as sock:
        ...
        set_arinc429rx = (rattr.pack(len(rattr) + len(avionics_arinc429rx), IFLA_AVIONICS_ARINC429RX) +
                avionics_arinc429rx.pack(flags, priority_labels, label_filters))
        info_data = rattr.pack(len(rattr) + len(set_arinc429rx), IFLA_INFO_DATA) + set_arinc429rx
