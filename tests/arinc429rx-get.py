...
if __name__ == "__main__":
    ...
    print(
            f"Flip Label Bits = {bool(config.flags&AVIONICS_ARINC429RX_FLIP_LABEL_BITS)}"
    )
    ...
    print(
            f"SD Mask Enable = {bool(config.flags&AVIONICS_ARINC429RX_SD_MASK_ENABLE)}"
    )
    print(
            f"Parity Check Enabled = {bool(config.flags&AVIONICS_ARINC429RX_PARITY_CHECK)}"
    )
    print(
            f"Even Parity = {bool(config.flags&AVIONICS_ARINC429RX_EVEN_PARITY)}"
    )
    print(
            f"Label Filter Enabled = {bool(config.flags&AVIONICS_ARINC429RX_LABEL_FILTER_ENABLE)}"
    )
    print(
            f"Priority Labels Enabled = {bool(config.flags&AVIONICS_ARINC429RX_PRIORITY_LABEL_ENABLE)}"
    )
