...
#define HI3717A_MAX_DATA	(HI3717A_FIFO_DEPTH * sizeof(__u32) * 8)
#define HI3717A_MTU		(sizeof(avionics_data) + HI3717A_MAX_DATA)
...
static void hi3717a_empty_fifo(struct hi3717a_priv *priv);
		...
		__u8 *dest, unsigned num_reads)
{
	...
	for(i = 0; i < num_reads; i++) {
		...
		val_be = cpu_to_be32(vbuffer);
		memcpy(&dest[i*sizeof(__u32)], &val_be, sizeof(__be32));
	}

	if(status < 0) {
		return status;
	} else {
		return num_reads * sizeof(__u32);
	}
}
...
{
	ssize_t status;
	__u8 dummy[HI3717A_FIFO_DEPTH * sizeof(__u32)];
	int i = 0;

	status = hi3717a_get_cntrl(priv, HI3717A_OPCODE_RD_RXFSTAT);

	while (status >= 0 && !(status & HI3717A_RXFIFO_EMPTY)) {
		hi3717a_rxfifo_read(priv, dummy, HI3717A_FIFO_DEPTH);

		status = hi3717a_get_cntrl(priv, HI3717A_OPCODE_RD_RXFSTAT);

		if (i++ > 10) {
			pr_err("avionics-hi3717a: Failed to clear RX FIFO\n");
			break;
		}
	}
}
				    ...
				    avionics_data *data)
{
	...
	dev = priv->dev;
	stats = &dev->stats;
	total_len = sizeof(avionics_data) + data->length;

	skb = avionics_device_alloc_skb(dev, total_len);
	if (unlikely(!skb)) {
		pr_err("avionics-hi3717a: Failed to allocate RX buffer\n");
		return -ENOMEM;
	}

	skb_copy_to_linear_data(skb, (void*)data, total_len);
...
static int hi3717a_rxfifo_read_all(struct hi3717a_priv *priv, __u8 *dest)
{
	...
	for (i = 0; i < HI3717A_FIFO_DEPTH; i++) {
		...
		memcpy(&dest[count * sizeof(__u32)], &val_be, sizeof(__be32));
		count++;
	}
					...
					struct net_device *dev)
{
	...
	data_hdr = (avionics_data *)skb->data;

	if (unlikely(data_hdr->length % 4)) {
		kfree_skb(skb);
		stats->tx_dropped++;
		return NETDEV_TX_OK;
	}
